...
import { Send, Plus, Trash2, Menu, User, Minimize2, ArrowLeft } from 'lucide-react'
...
export default function ChatInterface({ initialChatId, onToggleMode }: ChatInterfaceProps) {
  ...
  const startNewChat = () => {
    ...
    // Update URL to home without page reload
  ...
  const submitQuestion = async (questionText: string) => {
    ...
    try {
      ...
      // If this was a new chat, update the chat list and URL
  ...
  return (
    <div className="flex h-screen bg-white">
      ...
      <div className="flex-1 flex flex-col bg-white">
        {/* Header */}
        <header className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 shadow-md">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-semibold text-white">HR Assistant</h1>
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-2 rounded-full bg-green-300 animate-pulse"></div>
                    <p className="text-sm text-white/90">Online to assist you</p>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <div className="grid gap-3">
                      {selectedCategory.questions.map((q, idx) => (
                        <button
                          ...
                          className="w-full text-left px-5 py-3 bg-white border-2 border-green-200 hover:border-green-500 hover:bg-green-50 rounded-xl transition-all duration-200 text-gray-700 hover:text-green-700 font-medium shadow-sm hover:shadow-md"
                        >
                          {q}
                        </button>
                      ))}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
                    {faqCategories.map((category) => (
                      <button
                        ...
                        className="flex flex-col items-center gap-3 p-6 bg-white border-2 border-gray-200 hover:border-green-500 hover:bg-green-50 rounded-2xl transition-all duration-200 shadow-sm hover:shadow-lg group"
                      >
            ...
            {messages.length > 0 && !loading && (
              <div className="mt-8 pt-6 border-t border-gray-200">
                <p className="text-center text-gray-600 text-sm mb-4 font-medium">
                  {selectedCategory ? 'Choose another question:' : 'What else can I help you with? 👇'}
                </p>
                {selectedCategory ? (
                  <div className="max-w-3xl mx-auto">
                    <button
